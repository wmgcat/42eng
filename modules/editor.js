...
/*let Graphics = {
	...
	'ellipse': function(x, y, w, h, color, alpha, tp) {
		if (alpha != undefined) this.cvs.globalAlpha = alpha;
		if (color) this.cvs[(tp || 'fill') + 'Style'] = color;
		this.cvs.beginPath();
	...
	'gui': {
		'btn': function(x, y, w, h, col, img) {
			...
			if (Eng.collision.mouse.grect(x, y, w, h) && !Byte.check('hover')) {
				Byte.add('hover');
				hover = true;
			...
			Graphics.rect(x, y, w, h * .975, '#000000');
			if (!hover || !Byte.check('uclick')) Graphics.rect(x, y + h * .95, w, h * .05, '#000');
			if (img) {
		...
		'stick': function(x, y, range, col, back) {
			if (back == undefined || back === true) {
				Graphics.circle(x, y, range - 8, 0, Math.PI * 2, col || '#470009', .2);
				Graphics.cvs.lineWidth = 8;
				Graphics.circle(x, y, range, 0, Math.PI * 2, col || '#470009', .25, 'stroke');
			} else Graphics.circle(x, y, range * .2, 0, Math.PI * 2, col || '#470009', .2);
			let mx = mouse.touch.x, my = mouse.touch.y;
			let dist = Eng.math.clamp(Eng.math.distance(x, y, mx, my), -range, range),
				dir = Eng.math.direction(x, y, mx, my);
			Graphics.circle(x + dist * Math.cos(dir), y + dist * Math.sin(dir), Math.max(range * .5 * (dist / range), range * .35), 0, Math.PI * 2, col || '#470009', .2);
			Graphics.cvs.lineWidth = 1;
			return [dir, dist / range];
		},
		'dragWindow': function(id, x, y, w, h, header, content, headcol) {
			if (!memory['window.' + id]) memory['window.' + id] = { 'open': true, 'x': x, 'y': y }; // initialize window data
			if (header != undefined) {
				...
				if (Eng.collision.mouse.grect(memory['window.' + id].x + w * .8, memory['window.' + id].y - h * .1, w * .2, h * .1)) {
...
function Editor() {
	...
	if (memory.editor.dobj) {
		Add.gui(() => { // свойства объектов:
			let offset = cfg.grid * .25, w = cfg.window.width * .25, h = cfg.window.height * .5,
				x = cfg.window.width - w - offset, y = (cfg.window.height - h) * .5;
			if (memory.editor.dobj) {
				gr.rect(x, y, w, h, Style.black);
				gr.text(memory.editor.dobj.id, x + offset, y + offset, Style.white, 1, 20, 'Arial', 'fill', 'left-top');
			}
		});
	}
	Add.gui(() => { // управление на карте:
		if (Byte.check('dclick')) {
			mouse.save_x = mouse.x;
			mouse.save_y = mouse.y;
		}
		if ((mouse.save_x != undefined && mouse.save_y != undefined) && Eng.math.distance(mouse.x, mouse.y, mouse.save_x, mouse.save_y) >= cfg.grid) mouse.move = true;
		if (Byte.check('uclick')) {
			if (mouse.move) Byte.clear('uclick');
			delete mouse.save_x;
			delete mouse.save_y;
			delete mouse.move;
		}
		if (mouse.move) {
			let dir = Eng.math.direction(mouse.save_x, mouse.save_y, mouse.x, mouse.y),
				dist = Math.min(Eng.math.distance(mouse.save_x, mouse.save_y, mouse.x, mouse.y) / (cfg.grid * 4), 1);
			cameraes[current_camera].x -= Math.cos(dir) * dist * cfg.grid;
			cameraes[current_camera].y -= Math.sin(dir) * dist * cfg.grid;
		} else {
			if (Byte.check('uclick')) {
				let obj = Search.distance('all', mouse.x, mouse.y, cfg.grid * .5)[0];
				if (obj) {
					Add.debug('Выбран объект:', obj);
					memory.editor.dobj = obj;
					Byte.clear('uclick');
				} else delete memory.editor.dobj;
			}
		}
	});
...
function Editor(cvs, src, map) {
	...
	CONST = { 'wall': 0, 'floor': 1, 'steps': .001 };
	Add.gui(function(cvs) {
		if (Byte.check('dclick') && !Byte.check('hover') && !memory.creplace) {
			memory.save_mx = mouse.x * cfg.zoom, memory.save_my = mouse.y * cfg.zoom;
			memory.creplace = true;
			if (memory.editor.dobj) delete memory.editor.dobj;
			Byte.clear('dclick');
		}
		if (memory.creplace) {
			cameraes[current_camera].x += (memory.save_mx - mouse.x * cfg.zoom) * .1;
			cameraes[current_camera].y += (memory.save_my - mouse.y * cfg.zoom) * .1;
			if (Byte.check('uclick')) {
	...
	if (memory.editor && pause) {
		...
		if (memory.lobjects && memory.editor.window == 'objects') {
			for (let i = 0, obj; i < memory.lobjects.length; i++) { 
				...
				if (obj)
					if (Eng.collision.mouse.rect(obj.x, obj.y, cfg.grid) && !(Byte.check('hover') || memory.creplace)) {
						Byte.add('hover');
						if (Byte.check('dclick')) {
							memory.editor.dobj = obj;
							Byte.clear('dclick');
			...
			if (memory.editor.dobj) {
				...
				Add.gui(function(cvs) {
					...
					list.forEach(function(e, i) {
						...
						if (Eng.collision.mouse.grect(xx, ny + yy + (20 + yoffset) * i, cfg.grid * 5, 20) && !Byte.check('hover')) {
							gr.rect(xx, ny + yy + (20 + yoffset) * i, cfg.grid * 5, 20 + yy * 2, '#fff', .2);
							if(Byte.check('uclick')) {
								if (i == list.length - 1) {
		...
		switch (memory.editor.window) {
			case 'objects':
				Add.gui(function(cvs) {
					...
					gr.gui.dragWindow('object', offset, src.height * .075 + offset * 5, (msize + offset) * 2 + offset * 5, (msize + offset * 3) * 3 + offset, 'objects', function(left, top) {
						for (let i = (memory.editor_obj_page || 0) * 6, e; i < Math.min((memory.editor_obj_page || 0) * 6 + 6, memory.editor.objects.length); i++) {
							...
							cvs.textAlign = 'left';
							if (Eng.collision.mouse.grect(xx + left, yy + top, msize, msize) && !Byte.check('hover') && !memory.creplace) {
								Byte.add('hover');
								if (Byte.check('dclick')) {
									memory.editor_obj = e;
									Byte.clear('dclick');
					...
					if (memory.editor.objects.length > 6) {
						if ((memory.editor_obj_page || 0) < Math.floor(memory.editor.objects.length / 6))
							if (Eng.collision.mouse.grect(gr.rect((msize + offset) * 2 + offset * 4, src.height * .075 + offset * 5, offset * 2, (msize + offset * 3) * 3 + offset, '#000', .25)) && !Byte.check('hover') && !memory.creplace) {
								Byte.add('hover');
								if (Byte.check('uclick')) {
					...
					if (memory.editor_obj_page > 0) {
						if (Eng.collision.mouse.grect(gr.rect(offset, src.height * .075 + offset * 5, offset * 2, (msize + offset * 3) * 3 + offset, '#000', .25)) && !Byte.check('hover') && !memory.creplace) {
							Byte.add('hover');
							if (Byte.check('uclick')) {
								memory.editor_obj_page = (memory.editor_obj_page || 0) - 1;
								Byte.clear('uclick');
				...
				if (memory.editor_obj) {
					...
					if (Byte.check('uclick')) {
						Add.object(memory.editor_obj, Math.floor(xx / cfg.zoom / cfg.grid) * cfg.grid, Math.floor(yy / cfg.zoom / cfg.grid) * cfg.grid);
					...
					gr.gui.dragWindow('tiles', offset, src.height * .075 + offset * 5, (msize + offset) * 5 + offset * 5, (msize + offset * 3) * 3 + offset, 'tiles', function(left, top, w, h) {
						...
						if (!memory.editor_obj) {
							...
							for (let i = (memory.editor_obj_page || 0) * size, e; i < Math.min((memory.editor_obj_page || 0) * size + size, l.length); i++) {
								...
								if (Eng.collision.mouse.grect(left + 16, top + (i - size * (memory.editor_obj_page || 0)) * 24, 100, 24) && !Byte.check('hover')) {
									Byte.add('hover');
							...
							if (l.length > size) {
								if ((memory.editor_obj_page || 0) < Math.floor(l.length / size))
									if (Eng.collision.mouse.grect(gr.rect((msize + offset) * 5 + offset * 3, 0, offset * 2, (msize + offset * 3) * 3 + offset, '#000', .25)) && !Byte.check('hover') && !memory.creplace) {
										Byte.add('hover');
										if (Byte.check('uclick')) {
											memory.editor_obj_page = (memory.editor_obj_page || 0) + 1;
											Byte.clear('uclick');
							...
							if (memory.editor_obj_page > 0) {
								if (Eng.collision.mouse.grect(gr.rect(offset, 0, offset * 2, (msize + offset * 3) * 3 + offset, '#000', .25)) && !Byte.check('hover') && !memory.creplace) {
									Byte.add('hover');
									if (Byte.check('uclick')) {
										memory.editor_obj_page = (memory.editor_obj_page || 0) - 1;
										Byte.clear('uclick');
							...
							let height, ceil = cfg.grid, gridzoom = cfg.grid * cfg.zoom;
							if (memory.editor_obj != 'clear') {
								let round = [Math.min(images[memory.editor_obj].width * cfg.zoom, w), Math.min(images[memory.editor_obj].height * (Math.min((images[memory.editor_obj].width * cfg.zoom, w)) / (images[memory.editor_obj].width * cfg.zoom)) * cfg.zoom, cfg.grid * 5 * cfg.zoom)];
								ceil = Math.floor(round[0] / (images[memory.editor_obj].width / cfg.grid));
								height = Math.floor((images[memory.editor_obj].height) / cfg.grid) * ceil;
								gr.image(images[memory.editor_obj], 0, 24, round[0], height);
								//height = images[memory.editor_obj].height * cfg.zoom;
								//gr.image(images[memory.editor_obj], 0, 24, images[memory.editor_obj].width * cfg.zoom, height);
							} else height = 0;
							for (let i = 0, place = ['wall', 'floor', 'clear']; i < 3; i++) {
								...
								if (Eng.collision.mouse.grect(left, top + height + 48 + 24 * i, 100, 24) && !Byte.check('hover')) {
									Byte.add('hover');
							...
							switch(memory.editor_obj) {
								case 'clear':
									if (Byte.check('dclick') && !Byte.check('hover')) {
										let xx = Math.floor(mouse.x / cfg.zoom / cfg.grid), yy = Math.floor(mouse.y / cfg.zoom / cfg.grid), e = memory.editor_obj;
										val = CONST['wall'];
										//if (!nmap.memory[memory.editor_obj + nx + 'x' + ny]) nmap.registry(memory.editor_obj + nx + 'x' + ny, val, Img.init(memory.editor_obj, memory.select_tile.x * cfg.grid, memory.select_tile.y * cfg.grid, cfg.grid, cfg.grid));
								...
								break;
								default:
									if (Eng.collision.mouse.grect(left, top + 24, w, height) && !Byte.check('hover')) {
										Byte.add('hover');
										...
										if (Byte.check('dclick')) {
											...
											//console.log(memory.editor_obj + nx + 'x' + ny, nx, ny, cfg.grid, cfg.grid);
											Byte.clear('dclick');
										}
									}
									if (Eng.collision.mouse.grect(left, top, 100, 24) && !Byte.check('hover')) {
										Byte.add('hover');
				...
				Add.gui(function(cvs) {
					gr.gui.dragWindow('rooms', offset, src.height * .075 + offset * 5, (msize + offset) * 2 + offset * 5, (msize + offset * 3) * 3 + offset, 'rooms', function(left, top) {
						Object.keys(current_level.levels).forEach(function(e, i) {
							...
							if (Eng.collision.mouse.grect(left, top + 16 + 24 * i, 100, 16) && !Byte.check('hover')) {
								Byte.add('hover');
	...
	Add.gui(function(cvs) { // header:
		...
		gr.text(placeholder_level, xx - cfg.grid * 2, (src.height * .075) >> 1, '#fff', 1, 20, 'Consolas');
		cvs.textBaseline = 'top';
		cvs.textAlign = 'left';
